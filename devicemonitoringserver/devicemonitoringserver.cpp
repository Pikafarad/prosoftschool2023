...
void DeviceMonitoringServer::setDeviceWorkSchedule(const DeviceWorkSchedule& schedul)
{
        if(m_center)
            m_center->setWorkSchedule(&schedul);
}
...
void DeviceMonitoringServer::onMessageReceived(uint64_t deviceId, const std::string& message)
{
    ...
    sendMessage(deviceId, m_center->processingMeterage(message, deviceId));
